			...
			Если НЕ ТабЗаказы.Количество() = 0 Тогда
				...
				Для каждого СтрЗаказ Из ТабЗаказы Цикл
					...
					Для Каждого СтрТ Из ТЗ Цикл
						...
						НстрТЧ.Цена_2					= НстрТЧ.Цена*Курс; 
...
Процедура РаспределитьТоварыЗаказаКлиента_ПоПоставщикам()
	...
	Для каждого СтрТ из ТЗ_ цикл
		...
		Для Каждого СтрПроценки Из СтрокиПроценки Цикл
			...
			НаправлениеДоставкиЦенаЗаВес = СтрТ.ЗаказПокупателя.ИНФ_НаправлениеДоставкиЗаказа.Направление.ЦенаВесИзПрайса <> 0;
			НеУказанВес = НаправлениеДоставкиЦенаЗаВес И СтрПроценки.Вес = 0;  //Есть цена за вес, но в проценке вес не указан, возможен убыток при заказе 
			...
			Если НЕ ЦенаКлиента = 0 Тогда
				...
				ЦенаДоставки        = СтрПроценки.Вес * СтрТ.ЗаказПокупателя.ИНФ_НаправлениеДоставкиЗаказа.Направление.ЦенаВесИзПрайса * КурсВалютыЗаказа / ?(СтрТ.ЗаказПокупателя.ВалютаДокумента = ВалютаUSD, КурсВалютыЗаказа, 1) 
										+ СтрПроценки.Объем * СтрТ.ЗаказПокупателя.ИНФ_НаправлениеДоставкиЗаказа.Направление.ОбъемВесИзПрайса / КурсВалютыЗаказа * ?(СтрТ.ЗаказПокупателя.ВалютаДокумента = ВалютаUSD, КурсВалютыЗаказа, 1);
				ЦенаДоставки_2		= СтрПроценки.Вес * СтрТ.ЗаказПокупателя.ИНФ_НаправлениеДоставкиЗаказа.Направление.ЦенаВесИзПрайса * КурсВалютыUSD 
										+ СтрПроценки.Объем * СтрТ.ЗаказПокупателя.ИНФ_НаправлениеДоставкиЗаказа.Направление.ОбъемВесИзПрайса * КурсВалютыUSD;
		...
		Если СтрокаПроценки.Количество() = 1 Тогда
			...
			Если СтрокиРаспределения.Количество() > 0 Тогда
				...
				Если НЕ ЦенаКлиента = 0 Тогда
					...
					ЦенаДоставки        = Вес * ЗаказПокупателя.ИНФ_НаправлениеДоставкиЗаказа.Направление.ЦенаВесИзПрайса * КурсВалютыЗаказа / ?(РезобраноЗаказПокупателя.ВалютаДокумента = ВалютаUSD, КурсВалютыЗаказа, 1) 
											+ Объем * ЗаказПокупателя.ИНФ_НаправлениеДоставкиЗаказа.Направление.ОбъемВесИзПрайса * КурсВалютыЗаказа / ?(РезобраноЗаказПокупателя.ВалютаДокумента = ВалютаUSD, КурсВалютыЗаказа, 1);
					ЦенаДоставки_2		= Вес * ЗаказПокупателя.ИНФ_НаправлениеДоставкиЗаказа.Направление.ЦенаВесИзПрайса * КурсВалютыUSD
											+ Объем * ЗаказПокупателя.ИНФ_НаправлениеДоставкиЗаказа.Направление.ОбъемВесИзПрайса * КурсВалютыUSD;
...
ТЗ_РазобраноПоПоставщикам.Колонки.Добавить("ЦенаДоставки", Новый ОписаниеТипов("Число",КвалификаторыЧислаЦена),"Цена доставки");
